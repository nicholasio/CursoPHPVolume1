...
if ( $post_id ) {
	$msg = $post_type == 'page' ? 'Editar Página' : 'Editar Post';
	...
	$post_categories = wcms_db_select('post_categories', ['*'], ['posts_ID' => $post_id]);

	if ( $post_categories ) 
		$post_categories = array_map(function($cat) { return $cat->categories_ID; }, $post_categories);
	else 
		$post_categories = [];
} else {
	$post_categories = [];
...
<form class="form-horizontal" role="form" method="POST" action="<?= do_action('save_post'); ?>">
	...
	<div class="form-group">
		...
		<div class="col-md-8">
			<select name="post_author" class="form-control">
				...
				?>
				<option <?php 
					if ( ( ! $post_id && wcms_get_current_user_ID() == $user->ID) || ($post_author == $user->ID) ) 
						echo 'selected';
				?>
	...
	<?php if ( $post_type == 'post' ) : ?>
		<div class="form-group">
			...
			<div class="col-md-8">
				<select multiple name="post_categories[]" class="form-control">
					<?php 
					...
					if ( $categories) : ?>
						...
						?>	
							<option
							 <?php if ( in_array($cat->ID, $post_categories) ) echo 'selected'; ?>
